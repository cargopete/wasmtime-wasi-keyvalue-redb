...
This crate implements the same interface contract as the official `wasmtime-wasi-keyvalue` crate
(currently in-memory only) and can be used as a drop-in alternative wherever persistent storage
is needed — no external services required.

## Status

Work in progress. Targeting the [wasi:keyvalue Phase 2 proposal] portability criteria as a
reference implementation for embedded/single-node use cases.
...
[wasi:keyvalue Phase 2 proposal]: https://github.com/WebAssembly/wasi-keyvalue
